...
def test_laplacian_normalized_expected_output() -> None:
    ...
    actual: np.ndarray = laplacian_normalized_metric(_=np.empty(()), u=u)
    u /= 5.0
    np.testing.assert_allclose(actual=actual, desired=np.exp(-u).reshape((1, 50)))

    u = np.linspace(start=3, stop=4)
    actual = laplacian_normalized_metric(_=np.empty((100)), u=u)
    u /= 4.0
    np.testing.assert_allclose(actual=actual, desired=np.exp(-u).reshape((1, 50)))

    u = np.linspace(start=-2, stop=8)
    actual = laplacian_normalized_metric(_=np.ones(()), u=u)
    u /= 8.0
    np.testing.assert_allclose(actual=actual, desired=np.exp(-u).reshape((1, 50)))

    u = np.linspace(start=0, stop=5).reshape(1, 50)
    actual = laplacian_normalized_metric(_=np.ones((30)), u=u)
    u /= 5.0
    np.testing.assert_allclose(actual=actual, desired=np.exp(-u).reshape((1, 50)))

    u = np.linspace(start=3, stop=4).reshape(1, 50)
    actual = laplacian_normalized_metric(_=np.empty((12, 100)), u=u)
    u /= 4.0
    np.testing.assert_allclose(actual=actual, desired=np.exp(-u).reshape((1, 50)))

    u = np.linspace(start=-2, stop=8).reshape(1, 50)
    actual = laplacian_normalized_metric(_=np.zeros((12, 45)), u=u)
    u /= 8.0
    np.testing.assert_allclose(actual=actual, desired=np.exp(-u).reshape((1, 50)))
...
def test_tricube_normalized_expected_output() -> None:
    ...
    """
    u: np.ndarray = np.linspace(start=0, stop=5)
    actual: np.ndarray = tricube_normalized_metric(_=np.empty(()), u=u)
    ...
    actual = tricube_normalized_metric(_=np.zeros((30)), u=u)
    assert actual.min() == 0.0
    assert actual.max() == 1.0
    u /= 3.0
    desired = (1.0 - u**3) ** 3
...
def test_tricube_normalized_raises_when_inputs_negative() -> None:
    ...
    with pytest.raises(AssertionError):
        tricube_normalized_metric(_=np.empty(()), u=u)
