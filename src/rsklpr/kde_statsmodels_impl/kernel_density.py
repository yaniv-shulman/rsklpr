...
from rsklpr.kde_statsmodels_impl import kernels
from rsklpr.kde_statsmodels_impl.kernel_base import GenericKDE, EstimatorSettings, gpke, LeaveOneOut, _adjust_shape
...
class KDEMultivariate(GenericKDE):
    ...
    def pdf(self, data_predict=None):
        ...
        for i in range(np.shape(data_predict)[0]):
            pdf_est.append(
                gpke(self.bw, data=self.data, data_predict=data_predict[i, :], var_type=self.var_type) / self.nobs
            )
    ...
    def imse(self, bw):
        ...
        F = 0
        kertypes = dict(
            c=kernels.gaussian_convolution, o=kernels.wang_ryzin_convolution, u=kernels.aitchison_aitken_convolution
        )
        nobs = self.nobs
        data = -self.data
        var_type = self.var_type
...
class KDEMultivariateConditional(GenericKDE):
    ...
    def pdf(self, endog_predict=None, exog_predict=None):
        ...
        for i in range(np.shape(data_predict)[0]):
            f_yx = gpke(
                self.bw, data=self.data, data_predict=data_predict[i, :], var_type=(self.dep_type + self.indep_type)
            )
    ...
    def cdf(self, endog_predict=None, exog_predict=None):
        ...
        for i in range(N_data_predict):
            mu_x = (
                gpke(self.bw[self.k_dep :], data=self.exog, data_predict=exog_predict[i, :], var_type=self.indep_type)
                / self.nobs
            )
    ...
    def imse(self, bw):
        ...
        for ii, Z in enumerate(zLOO):
            ...
            K_Xi_Xl = gpke(
                bw[self.k_dep :], data=Xe_L, data_predict=self.exog[ii, :], var_type=self.indep_type, tosum=False
            )
            K_Xj_Xl = gpke(
                bw[self.k_dep :], data=Xe_R, data_predict=self.exog[ii, :], var_type=self.indep_type, tosum=False
            )
            K2_Yi_Yj = gpke(
                bw[0 : self.k_dep],
                data=Ye_L,
