...
def _compute_subset(class_type, data, bw, co, do, n_cvars, ix_ord, ix_unord, n_sub, class_vars, randomize, bound):
    ...
    if class_type == "KDEMultivariate":
        ...
        raise ValueError(
            "class_type not recognized, should be one of " "{KDEMultivariate, KDEMultivariateConditional, KernelReg}"
        )
...
class GenericKDE(object):
    ...
    def _cv_ml(self):
        ...
        bw = optimize.fmin(self.loo_likelihood, x0=h0, args=(np.log,), maxiter=1e3, maxfun=1e3, disp=0, xtol=1e-3)
...
class EstimatorSettings:
    ...
    def __init__(
        self, efficient=False, randomize=False, n_res=25, n_sub=50, return_median=True, return_only_bw=False, n_jobs=-1
    ):
...
def gpke(
    bw, data, data_predict, var_type, ckertype="gaussian", okertype="wangryzin", ukertype="aitchisonaitken", tosum=True
):
    r"""
