...
def benchmark_curve_1(
    noise_ratio: float, hetero: bool, num_points: int
    ...
    if hetero:
        scale = generator.uniform(
            low=0.001, high=noise_ratio * (y_true.max() - y_true.min()), size=x.shape[0]
        )
    else:
        scale = np.full(
            shape=x.shape[0], fill_value=noise_ratio * (y_true.max() - y_true.min())
        )
...
def benchmark_curve_3(
    ...
    y_true: np.ndarray = (
        5 * x
        + np.square(x)
        + np.square(np.square(np.power(x, 3)))
        + (1.3 * x / np.max(x) * np.sin(x * -2.5 * np.pi))
    )
