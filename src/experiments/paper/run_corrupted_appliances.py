...
from experiments.paper.run_public_appliances import (  # noqa: E402
    COLORS,
    DATA_FILE,
    DATA_URLS,
    KERNEL_LABELS,
...
from rsklpr.kernels import laplacian_normalized_metric, tricube_normalized_metric  # noqa: E402
...
def clean_metric_rows(y_true_clean: np.ndarray, y_pred: np.ndarray, runtime_s: float) -> dict[str, float | bool]:
...
def fit_manual_search(
    ...
    def evaluate_fold(params: dict[str, Any], fold: int, inner_train: np.ndarray, inner_val: np.ndarray) -> dict[str, Any]:
    ...
    for repeat in range(config.holdout_repeats):
        ...
        for noise_type in config.noise_types:
            for noise_level in config.noise_levels:
                ...
                print(
                    f"\n  noise={noise_type}, level={noise_level:g}, "
                    f"noise_std={np.nanstd(train_noise):.3f}",
                    flush=True,
                )
...
def summarize(test_results: pd.DataFrame) -> pd.DataFrame:
    ...
    winners["is_winner"] = winners["rmse"] == winners.groupby(
        ["dataset", "noise_type", "noise_level", "repeat"]
    )["rmse"].transform("min")
...
def summarize_relative(summary: pd.DataFrame) -> pd.DataFrame:
    baseline = summary[summary["model"] == "lowess"][
        ["noise_type", "noise_level", "rmse_mean", "mae_mean"]
    ].rename(columns={"rmse_mean": "lowess_rmse_mean", "mae_mean": "lowess_mae_mean"})
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", default="corrupted_appliances", help="Relative subdirectory under $REPO_DIR/out.")
