...
it follows that:
\begin{align}
\hat{\beta}(x) &\coloneqq \min_{\beta(x)} \left( \sum_{i=1}^N \hat{f}(X_i, Y_i) \right)^{-1} \sum_{i=1}^N \left( Y_i - \sum_{j=0}^p \beta_j(x) (x - X_i)^j \right)^2 K_{H_1}(x - X_i) \hat{f}(X_i, Y_i) \\
&= \min_{\beta(x)} \left( \sum_{i=1}^N \hat{f}(X_i, Y_i) \right)^{-1} \sum_{i=1}^N g(X_i, Y_i) \hat{f}(X_i, Y_i) \\
&= \min_{\beta(x)} \left( \frac{1}{N} \sum_{i=1}^N \hat{f}(X_i, Y_i) \right)^{-1} \frac{1}{N} \sum_{i=1}^N g(X_i, Y_i) \hat{f}(X_i, Y_i)
\end{align}

As \( N \to \infty \), by the law of large numbers and the continuous mapping theorem, we obtain:
\begin{align}
\lim_{N \to \infty} \left( \frac{1}{N} \sum_{i=1}^N \hat{f}(X_i, Y_i) \right)^{-1} &\xrightarrow{a.s.} \mathbb{E} \left[ \frac{1}{\hat{f}(X, Y)} \right] \\
\lim_{N \to \infty} \frac{1}{N} \sum_{i=1}^N g(X_i, Y_i) \hat{f}(X_i, Y_i) &\xrightarrow{a.s.} \mathbb{E} \left[ g(X, Y) \hat{f}(X, Y) \right]
\end{align}

Assuming \( \mathbb{E}[\hat{f}(X, Y)] \neq 0 \), it follows that:
\begin{align}
\hat{\beta}(x) &\coloneqq \min_{\beta(x)} \frac{\mathbb{E} \left[ g(X, Y) \hat{f}(X, Y) \right]}{\mathbb{E} \left[ \hat{f}(X, Y) \right]}
\end{align}
If \( g(X, Y) \) and \( \hat{f}(X, Y) \) are uncorrelated, then:
\begin{align}
\hat{\beta}(x) &\coloneqq \min_{\beta(x)} \frac{\mathbb{E} \left[ g(X, Y) \hat{f}(X, Y) \right]}{\mathbb{E} \left[ \hat{f}(X, Y) \right]} = \min_{\beta(x)} \mathbb{E} \left[ g(X, Y) \right]
\end{align}
Thus, as \( N \to \infty \):
\begin{align}
\hat{\beta}(x) &= \min_{\beta(x)} \lim_{N \to \infty} \frac{1}{N} \sum_{i=1}^N g(X_i, Y_i) \\
&= \min_{\beta(x)} \lim_{N \to \infty} \frac{1}{N} \sum_{i=1}^N \left( Y_i - \sum_{j=0}^p \beta_j(x) (x - X_i)^j \right)^2 K_{H_1}(x - X_i)
\end{align}

By removing the $ \frac{1}{N} $ term from the summation in the limiting case, we recover the standard LPR estimator. This is because the optimization is invariant to scale, and removing $ \frac{1}{N} $ simply leads to a minimization of the sum of squared residuals, which is the essence of the standard LPR. Thus, the proposed estimator degenerates to the standard LPR in this finite-sample setting as $ N \to \infty $, provided the sum does not diverge.

The goal of the proposed method is to prevent the degeneration to standard LPR by ensuring that the weights $ f(X,Y) $ are correlated with the residuals. If $ f(X,Y) $ becomes uncorrelated with $ g(X,Y) $, the weighting scheme would lose its intended robustness, treating all points equally, and the estimator would revert to the standard LPR, losing the benefits of outlier mitigation. The method is designed to avoid this by using similarity kernels that ensure the weight function $ f(X,Y) $ reflects the local structure of the data, with the expectation that large residuals are indicative of outliers or lower density regions.
