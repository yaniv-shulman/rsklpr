...
The proposed method (RSKLPR) is implemented in Python and published as an open source package \href{https://github.com/yaniv-shulman/rsklpr}{https://github.com/yaniv-shulman/rsklpr}. This section reports two complementary empirical studies. First, a public regression benchmark evaluates predictive performance under cross-validated hyperparameter selection. Second, a synthetic target-validation experiment tests whether the empirical behaviour agrees with the population-target analysis in Section~\ref{S:Properties}.
...
All methods used the same local neighborhood size at each training-set size. Specifically, the number of neighbors was set to
\[
N_{\mathrm{loc}}=\min\{T,\max(15,\min(200,\mathrm{round}(0.25T)))\},
\]
where $T$ is the number of training observations. For the LOWESS baselines this was converted to the Statsmodels span parameter $\mathrm{frac}=N_{\mathrm{loc}}/T$; robust LOWESS used five residual reweighting iterations. Thus, the robust LOWESS baseline was not run with the Statsmodels default span $\mathrm{frac}=2/3$. Additional implementation details are provided in~\ref{appendix:experimental_hyperparameters}.
...
The LOWESS and robust LOWESS baselines were computed using the Statsmodels implementation in one dimension. Standard LOWESS used the tricube distance kernel with no residual reweighting (\texttt{it=0}); robust LOWESS used the same span with five bisquare residual-reweighting iterations (\texttt{it=5}). For both LOWESS variants the span was set to $\mathrm{frac}=N_{\mathrm{loc}}/T$, so the comparison used the same effective local sample size as RSKLPR.
