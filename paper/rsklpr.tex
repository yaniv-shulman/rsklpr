...
The proposed estimator is equivalent to using weighted arithmetic mean of the terms in the standard LPR loss \eqref{loss_lpr} with weights $w_i = \hat{f}(Y_i \, | \, X_i)$. \newline
\textit{Proof}: The optimization is invariant to the scale of the objective, therefore:
\begin{align}
\hat{\beta}(x,y) \coloneqq & \min_{\beta(x,y)} \sum_{i=1}^N \left(  Y_i - \sum_{j=0}^p \beta_j (x, y) (x - X_i)^j \right)^2 K_{H_{1}} (x - X_i) \hat{f}(y \, | \, x) \hat{f}(Y_i \, | \, X_i) \\
= & \min_{\beta(x,y)} \left( \sum_{i=1}^N \hat{f}(Y_i \, | \, X_i) \right)^{-1} \sum_{i=1}^N \left(  Y_i - \sum_{j=0}^p \beta_j (x, y) (x - X_i)^j \right)^2 K_{H_{1}} (x - X_i) \hat{f}(Y_i \, | \, X_i) \\
= & \min_{\beta(x,y)} \left( \sum_{i=1}^N w_i \right)^{-1} \sum_{i=1}^N \left(  Y_i - \sum_{j=0}^p \beta_j (x, y) (x - X_i)^j \right)^2 K_{H_{1}} (x - X_i) w_i
\label{alt_skr_opt_obj}
\end{align}

\subsection{Asymptotic degeneration of the estimator to the standard LPR}
Asymptotically, the proposed estimator degenerates to the standard LPR when the weights $w_i$ are uncorrelated to the standard LPR terms. Formally, $\hat{\gamma}(x) = \hat{\beta}(x,y)  \; \forall y$ when \newline $\left(  Y - \sum_{j=0}^p \beta_j (x, y) (x - X)^j \right)^2 K_{H_{1}} (x - X)$ and $\hat{f}(Y \, | \, X)$ are uncorrelated. \newline
\textit{Proof}: Define $g(X,Y) \coloneqq \left(  Y - \sum_{j=0}^p \beta_j (x, y) (x - X)^j \right)^2 K_{H_{1}} (x - X)$, it then follows that: 

\begin{align}
\hat{\beta}(x,y) \coloneqq & \min_{\beta(x,y)} \left( \sum_{i=1}^N \hat{f}(Y_i \, | \, X_i) \right)^{-1}  \sum_{i=1}^N \left(  Y_i - \sum_{j=0}^p \beta_j (x, y) (x - X_i)^j \right)^2 K_{H_{1}} (x - X_i) \hat{f}(Y_i \, | \, X_i) \\
= & \min_{\beta(x,y)} \left( \sum_{i=1}^N \hat{f}(Y_i \, | \, X_i) \right)^{-1}  \sum_{i=1}^N g(X_i , Y_i) \hat{f}(Y_i \, | \, X_i) \\
= & \min_{\beta(x,y)} \left( \frac{1}{N} \sum_{i=1}^N \hat{f}(Y_i \, | \, X_i) \right)^{-1} \frac{1}{N} \sum_{i=1}^N g(X_i , Y_i) \hat{f}(Y_i \, | \, X_i)
\end{align} \newline
When $N \rightarrow \infty$, due to the law of large numbers and the continuous mapping theorem it follows that:

\begin{align}
\lim_{N \rightarrow \infty} \left( \frac{1}{N} \sum_{i=1}^N \hat{f}(Y_i \, | \, X_i) \right)^{-1} 
& \xrightarrow{a.s.} \mathbb{E} \left[ \frac{1}{\hat{f}(Y \, | \, X)} \right] \\
\lim_{N \rightarrow \infty} \frac{1}{N}  \sum_{i=1}^N g(X , Y) \hat{f}(Y_i \, | \, X_i) & \xrightarrow{a.s.} \mathbb{E} \left[ g(X , Y) \hat{f}(Y \, | \, X) \right]
\end{align}
Combining the results above and assuming $\mathbb{E} \left[ \hat{f}(Y \, | \, X) \right] \neq 0$ it follows that:
\begin{align}
\hat{\beta}(x,y) \coloneqq \min_{\beta(x,y)} \frac{\mathbb{E} \left[ g(X , Y) \hat{f}(Y \, | \, X) \right]}{\mathbb{E} \left[ \hat{f}(Y \, | \, X) \right]}
\end{align}
If $g(X,Y)$ and $\hat{f}(Y \, | \, X)$ are uncorrelated then:
\begin{align}
\hat{\beta}(x,y) \coloneqq & \min_{\beta(x,y)} \frac{\mathbb{E} \left[ g(X , Y) \hat{f}(Y \, | \, X) \right]}{\mathbb{E} \left[ \hat{f}(Y \, | \, X) \right]} = \min_{\beta(x,y)} \frac{\mathbb{E} \left[ g(X , Y) \right] \mathbb{E} \left[ \hat{f}(Y \, | \, X) \right]}{\mathbb{E} \left[ \hat{f}(Y \, | \, X) \right]} = \min_{\beta(x,y)} \mathbb{E} \left[ g(X , Y) \right] \\
= & \min_{\beta(x,y)} \lim_{N \rightarrow \infty} \frac{1}{N}  \sum_{i=1}^N g(X , Y) \\
= & \min_{\beta(x,y)} \lim_{N \rightarrow \infty} \frac{1}{N}  \sum_{i=1}^N \left(  Y_i - \sum_{j=0}^p \beta_j (x, y) (x - X_i)^j \right)^2 K_{H_{1}} (x - X)
\label{lim_beta}
\end{align}
Since the last equation \eqref{lim_beta} is independent of the value of $y$, it is equivalent to the standard LPR as per equation \eqref{loss_lpr}:
\begin{align}
\min_{\gamma(x)} \lim_{N \rightarrow \infty} \frac{1}{N} \sum_{i=1}^N \left(  Y_i - \sum_{j=0}^p \gamma_j (x) (x - X_i)^j \right)^2 K_{H_{1}} (x - X_i) = \hat{\gamma}(x)
\end{align}
