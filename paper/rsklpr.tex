...
\]
and assume that the first centered weighted design moment vanishes:
\begin{equation}
M_1(x)
=\iint
(u-x)K_{h_1}(x-u)\,
w(u,v)\,
f_{X,Y}(u,v)\,\mathrm d v\,\mathrm d u
=0. \label{eq:M1_centering}
\end{equation}
This condition is satisfied, for example, when the kernel is symmetric and the effective weighted design density
\[
A(u)=\int w(u,v)f_{X,Y}(u,v)\,\mathrm d v
\]
is locally symmetric around $x$. More generally, it should be viewed as a local centering/symmetry condition, not as a generic assumption. Under this local-linear centering condition, the population minimizer also satisfies \eqref{eq:pop_intercept}.
\end{proposition}
...
For higher-order local polynomials ($p \ge 2$), the intercept is generally given by the first component of the weighted normal-equation solution and does not reduce to the simple ratio above without stronger structure. We therefore restrict the explicit ratio characterization to the local constant case and to the locally centered local-linear case.
