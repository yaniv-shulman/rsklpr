...
\subsection{Asymptotic Equivalence in Expected Loss Function to Standard LPR under the Normality Assumption}
\label{S}

In this section, a detailed asymptotic analysis of the proposed estimator is provided under the assumption that the conditional distribution \( f(Y \mid X) \) is normal. The goal is to demonstrate that, under certain conditions, the expected loss function simplifies to a form analogous to that of standard local polynomial regression (LPR).
...
\begin{enumerate}[label=(A\arabic*)]
    \item \textbf{Independent and Identically Distributed Data:} The data points $\{(X_i, Y_i)\}_{i=1}^N$ are independent and identically distributed (i.i.d.) samples from the joint distribution $f(X, Y)$.

    \item \textbf{Kernel Functions:}
    \begin{enumerate}[label=(\alph*)]
        \item The predictor $X$ is univariate. This simplifies the analysis; extension to multivariate $X$ would require a bandwidth matrix $\mathbf{H}$.
        
        \item The kernel $K_1(u)$ is a symmetric, bounded, and integrable function satisfying $\int_{-\infty}^\infty K_1(u) \, du = 1$.
        
        \item The bandwidth $h$ for $K_1$ satisfies $h \to 0$ and $N h \to \infty$ as $N \to \infty$.
        
        \item The similarity kernel $K_2\left( (x, Y_i), (X_i, Y_i); H_2 \right)$ is proportional to an estimate of the conditional density:
        \[
        K_2\left( (x, Y_i), (X_i, Y_i); H_2 \right) \propto \hat{f}(Y_i \mid X_i),
        \]
        where $\hat{f}(Y_i \mid X_i)$ is a kernel density estimator of the conditional density of $Y$ given $X$, which converges to $f(Y_i \mid X_i)$ as $N \to \infty$.
    \end{enumerate}

    \item \textbf{Conditional Normality:} The conditional distribution  \( f( Y \mid X = x) \) is normally distributed with mean $m(X)$ and variance $\sigma^2(X)$:
    \[
   f(   Y \mid X = x ) \sim \mathcal{N}\left( m(X), \sigma^2(X) \right).
    \]

    \item \textbf{Smoothness of Functions:} The regression function $m(X)$ and the variance function $\sigma^2(X)$ are twice continuously differentiable in a neighborhood of $x$.

    \item \textbf{Finite and Positive Variance:} The variance $\sigma^2(X)$ is finite and strictly positive for all $X$ in the domain.

\end{enumerate}

\subsubsection*{\textbf{Expected Loss Function}}

Recall that the proposed loss function is:

\begin{align}
\mathcal{L}_{rsk}(x) = \sum_{i=1}^N \left( Y_i - \beta(X_i; x) \right)^2 K_1\left( \frac{X_i - x}{h} \right) K_2\left( (x, Y_i), (X_i, Y_i); H_2 \right), \label{eq:loss}
\end{align}
where
\[
\beta(X_i; x) = \sum_{j=0}^p \beta_j(x)(X_i - x)^j.
\]

Under assumption (A2d), $K_2\left((x, Y_i), (X_i, Y_i); H_2\right) \propto f(Y_i \mid X_i)$ as $N \to \infty$. For the purpose of asymptotic analysis, it is assumed that $N \to \infty$ and that $K_2$ is proportional to $f(Y_i \mid X_i)$. Therefore, the expected loss function can be expressed as:

\begin{align}
\mathbb{E}\left[ \mathcal{L}_{rsk}(x) \right] = N \iint \left( Y - \beta(X; x) \right)^2 K_1\left( \frac{X - x}{h} \right) K_2\left( (x, Y), (X, Y); H_2 \right) f(X, Y) \, dY \, dX. \label{eq:expected_loss}
\end{align}

Substituting $K_2 \propto f(Y \mid X)$ and $f(X, Y) = f(Y \mid X) f(X)$, we have:

\begin{align}
\mathbb{E}\left[ \mathcal{L}_{rsk}(x) \right] \propto N \iint \left( Y - \beta(X; x) \right)^2 K_1\left( \frac{X - x}{h} \right) \left[f(Y \mid X)\right]^2 f(X) \, dY \, dX, \label{eq:simplified_loss}
\end{align}

which further simplifies to:

\begin{align}
\mathbb{E}\left[ \mathcal{L}_{rsk}(x) \right] \propto N \int f(X) K_1\left( \frac{X - x}{h} \right) \int \left( Y - \beta(X; x) \right)^2  \left[f(Y \mid X)\right]^2  \, dY \, dX, \label{eq:further_simplified_loss}
\end{align}

\subsubsection*{Simplifying the Inner Integral of $Y$}

Under assumption (A3), $f (Y \mid X )$ follows a normal distribution:

\begin{align}
f(Y \mid X) = \frac{1}{\sqrt{2\pi \sigma^2(X)}} \exp\left( -\frac{(Y - m(X))^2}{2 \sigma^2(X)} \right). \label{eq:conditional_density}
\end{align}

Therefore, the squared conditional density is:

\begin{align}
\left[f(Y \mid X)\right]^2 = \left( \frac{1}{\sqrt{2\pi \sigma^2(X)}} \right)^2 \exp\left( -\frac{(Y - m(X))^2}{\sigma^2(X)} \right). \label{eq:squared_density}
\end{align}

This is proportional to a normal density with mean $m(X)$ and variance $\sigma^2(X)/2$:

\begin{align}
\left[f(Y \mid X)\right]^2 \propto \phi\left( Y; m(X), \frac{\sigma^2(X)}{2} \right), \label{eq:scaled_density}
\end{align}
...
\begin{align}
E_Y = \int \left( Y - \beta(X; x) \right)^2 \left[f(Y \mid X)\right]^2 \, dY. \label{eq:inner_integral}
\end{align}

Since $\left[f(Y \mid X)\right]^2$ is proportional to $\phi\left(Y; m(X), \sigma^2(X)/2\right)$, we can evaluate $E_Y$ using the expectation of square deviations under the normal distribution:

\begin{align}
E_Y = \left( m(X)- \beta(X; x) \right)^2 + \frac{\sigma^2(X)}{2}. \label{eq:inner_result}
\end{align}

\subsubsection*{Simplifying the Expected Loss Function}

Substituting \( E_Y \) back into equation \eqref{eq:simplified_loss}, we have:

\begin{align}
\mathbb{E}\left[ \mathcal{L}_{rsk}(x) \right] \propto N \int f(X) K_1\left( \frac{X - x}{h} \right)  \left( \left( m(X) - \beta(X; x) \right)^2 + \frac{\sigma^2(X)}{2} \right) \, dX. \label{eq:final_loss}
\end{align}

Since \( \frac{\sigma^2(X)}{2} \) does not depend on \( \beta_j(x) \), minimizing \( \mathbb{E}[\mathcal{L}_{rsk}(x)] \) with respect to \( \beta_j(x) \) is equivalent to minimizing:

\begin{align}
J(\beta(x)) = \int f(X) K_1\left( \frac{X - x}{h} \right)  \left( m(X) - \beta(X; x) \right)^2 \, dX. \label{eq:objective}
\end{align}

This is the same objective function minimized by the standard LPR in expectation. Therefore, under the normality assumption and as \( N \to \infty \), the expected loss functions of the proposed method and the standard LPR converge, indicating that both methods target the same regression function \( m(x) \) on average.

\subsubsection*{Implications and Discussion}

The convergence of the expected loss functions suggests that the robust method and standard LPR aim to estimate the same underlying regression function asymptotically. However, this equivalence in expectation does not imply identical behavior in distribution. The robust method’s weighting scheme, which adjusts the influence of data points based on their estimated densities, affects the variability of the estimator.

By down-weighting outliers and points in regions of low density, the robust method can exhibit different variance properties, potentially improving robustness and efficiency in finite samples, particularly under heteroscedastic noise or the presence of outliers. This variance adjustment underscores the practical advantage of the robust method but also highlights the need for further theoretical analysis to fully characterize its asymptotic variance and distribution.
...
\begin{align}
\mathbb{E}\left[ \mathcal{L}_{\text{rsk}}(x) \right] = N \iint \left( Y - \beta(X; x) \right)^2 K_1\left( \frac{X - x}{h} \right) K_2\left( (x, Y), (X, Y); H_2 \right) f(X, Y) \, dY \, dX. \label{eq:expected_loss_asym}
\end{align}

Assuming that \( K_2 \propto f(Y \mid X) \) and recognizing that \( f(X, Y) = f(Y \mid X) f(X) \), the expected loss simplifies to:
...
\appendix
\section{Asymptotic Bias Example with Exponential Conditional Distribution} \label{appendix:asym_bias_example}

In this appendix, we provide a detailed example illustrating how asymmetry in the conditional distribution \( f(Y \mid X) \) can introduce asymptotic bias in the proposed estimator due to the squared density weighting. Specifically, we consider the case where the conditional distribution of \( Y \) given \( X \) is exponential, a common asymmetric distribution.

\subsection{Setup of the Example}

Suppose that for each fixed \( X \), the conditional distribution \( Y \mid X \) follows an exponential distribution shifted by \( m(X) \):

\begin{align}
f(Y \mid X) = \lambda(X) \exp\left( -\lambda(X) (Y - m(X)) \right), \quad \text{for } Y \geq m(X),
\end{align}

where \( \lambda(X) > 0 \) is the rate parameter, and \( m(X) \) is the location parameter (shift), which represents the true regression function we aim to estimate.

The mean and variance of this distribution are:

\begin{align}
\mathbb{E}[Y \mid X] &= m(X) + \frac{1}{\lambda(X)}, \\
\operatorname{Var}[Y \mid X] &= \frac{1}{\lambda^2(X)}.
\end{align}

\subsection{Computing the Squared Density}

The squared conditional density is:

\begin{align}
[f(Y \mid X)]^2 = \left( \lambda(X) \right)^2 \exp\left( -2\lambda(X) (Y - m(X)) \right), \quad \text{for } Y \geq m(X).
\end{align}

This squared density is proportional to an exponential distribution with rate parameter \( 2\lambda(X) \):

\begin{align}
g(Y \mid X) = \frac{[f(Y \mid X)]^2}{\int_{m(X)}^\infty [f(u \mid X)]^2 \, du} = 2\lambda(X) \exp\left( -2\lambda(X) (Y - m(X)) \right), \quad \text{for } Y \geq m(X).
\end{align}

\subsection{Calculating the Mean and Variance under the Squared Density}

The mean and variance of \( Y \) under the squared density \( g(Y \mid X) \) are:

\begin{align}
\mu'(X) &= \mathbb{E}_{g}[Y \mid X] = m(X) + \frac{1}{2\lambda(X)}, \\
\sigma'^2(X) &= \operatorname{Var}_{g}[Y \mid X] = \frac{1}{(2\lambda(X))^2}.
\end{align}

\subsection{Deriving the Asymptotic Bias}

As per the analysis in Section~\ref{s:AsymptoticBias}, the expected loss function simplifies to:

\begin{align}
\mathbb{E}\left[ \mathcal{L}_{\text{rsk}}(x) \right] \propto N \int K_1\left( \frac{X - x}{h} \right) f(X) \left( \left( \mu'(X) - \beta(X; x) \right)^2 + \sigma'^2(X) \right) \, dX.
\end{align}

Minimizing with respect to \( \beta(X; x) \) leads to the estimator converging to \( \beta(X; x) = \mu'(X) \). Therefore, the asymptotic bias at point \( x \) is:

\begin{align}
\text{Bias}(x) &= \mu'(x) - m(x) \\
&= \left( m(x) + \frac{1}{2\lambda(x)} \right) - m(x) \\
&= \frac{1}{2\lambda(x)}.
\end{align}

This expression shows that the estimator is asymptotically biased upwards by \( \frac{1}{2\lambda(x)} \) compared to the true regression function \( m(x) \).

\subsection{Interpretation}

The bias arises because the weighting induced by \( [f(Y \mid X)]^2 \) effectively shifts the mean of the distribution used in the expected loss function. In the case of the exponential distribution, squaring the density doubles the rate parameter from \( \lambda(X) \) to \( 2\lambda(X) \), reducing the mean from \( m(X) + \frac{1}{\lambda(X)} \) to \( m(X) + \frac{1}{2\lambda(X)} \).

This shift means that the estimator targets \( \mu'(X) \) instead of \( m(X) \), resulting in an asymptotic bias proportional to \( \frac{1}{2\lambda(X)} \).

\subsection{Numerical Example}

For illustrative purposes, consider \( \lambda(X) = 1 \) for all \( X \). Then, the bias simplifies to:

\begin{align}
\text{Bias}(x) = \frac{1}{2}.
\end{align}

In this case, the estimator is asymptotically biased upwards by \( 0.5 \) units at every point \( x \).

\subsection{Implications for the Estimator}

This example demonstrates that when the conditional distribution \( f(Y \mid X) \) is asymmetric, the proposed estimator may not converge to the true regression function \( m(X) \) as \( N \to \infty \), but rather to a biased version shifted by \( \mu'(X) - m(X) \).

In practice, the magnitude of the bias depends on the degree of asymmetry and the rate parameter \( \lambda(X) \). For large \( \lambda(X) \), the bias diminishes, and the estimator approaches \( m(X) \). However, for small \( \lambda(X) \), the bias becomes more significant.

\subsection{Conclusion}

This example illustrates how asymmetry in the conditional distribution \( f(Y \mid X) \) can introduce asymptotic bias in the proposed estimator due to the squared density weighting. It underscores the importance of considering the nature of the conditional distribution when applying the robust estimator and highlights the potential trade-off between robustness to outliers and asymptotic bias.

In practical applications, one may need to assess whether the benefits of robustness outweigh the potential bias introduced, especially in cases where the conditional distribution is significantly asymmetric.
